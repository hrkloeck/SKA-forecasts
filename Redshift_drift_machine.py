...
z_val      = [0.2, 0.3, 0.4, 0.5]   # redshifts
delta_z    = 0.1                         
...
#
#dvplt.plot_Nz(z_val, Dnu_val, S_area_val, t_obs_def, N_ant_def,delta_z)


# === Plot colour image of error estimates (image dimension sky area (horizontal) and channel width (vertical)) ===
#
#dvplt.im_sigmav(z_eg, Dnu_val, S_area_val, t_obs_def, N_ant_def, fwhm_def,delta_z,doprtinfo=False,doplot=True)
...
#
#dvplt.im_vsignificance(z_eg, Dnu_val[0], Dnu_val[-1], S_area_val[0], S_area_val[-1], t_obs_def, t_exp_def, N_ant_def, fwhm_def, p_LCDM, delta_z)


# === Plot diagram error estimates versus redshift per sky area  ===
#
# dvplt.plot_sigmav(z_val, Dnu_val, S_area_val, t_obs_def, N_ant_def, fwhm_def, delta_z)


# === Plot diagram significance of error estimates/theoretical model versus redshift per sky area  ===
#
#dvplt.plot_vsignificance(z_val, Dnu_val, S_area_val, t_obs_def, t_exp_def, N_ant_def, fwhm_def, p_LCDM, delta_z)

# === Plot diagram theoretical model redshiftdrifts and uncertainties versus redshift per sky area  ===
#
#dvplt.plot_Dv(z_val, Dnu_val, S_area_val, t_obs_def, t_exp_def, N_ant_def, fwhm_def, p_LCDM, delta_z)


# === Plot confidence ellipses  ===
#
#dvplt.plot_ellps_panel(np.array([0.1, 0.3, 0.5]), Dnu_val, S_area_val, t_obs_def, t_exp_def, N_ant_def, fwhm_def, p_LCDM, delta_z,  priors_baseline, savefig=True)

#dvplt.plot_ellipses(np.array([0.1, 0.3, 0.5]), Dnu_val, 5000, t_obs_def, t_exp_def, N_ant_def, fwhm_def, p_LCDM, delta_z, priors_baseline, savefig=True)

# === Plot the final image of the SKA Book  ===
#
dvplt.plot_vsignificance_as_error_for_SKA_book(z_val, Dnu_val, S_area_val, t_exp_def, p_LCDM, t_obs_def, N_ant_def, fwhm_def,delta_z,priors_baseline,savefig=True)
...
#fom_val_t = [analysis(t_obs, t_exp_def, N_ant_def, best_dnu, best_area, fwhm_def, priors_baseline, rtrnFoM=True) for t_obs in t_obs_val]
